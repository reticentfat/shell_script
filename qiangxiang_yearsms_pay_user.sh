...
TARGET_DIR='/data0'/$V_YEAR/$V_DATE
#取上一个月的最后一天
#判断是否是最后一天
v_day=$(gawk 'BEGIN{
             t="'$v_month'01000000"
...
CODE_DIR='/data0/match/orig/profile'
  ...
  # 数据文件
   ...
   bzcat /data0/match/orig/${V_DATE}/snapshot.txt.bz2 | grep  -e '|10301093|'  -e '|103240' | awk -F '|' -v first_date='20100701000000'   '{ if(  $3 == "06" && $5 >= first_date )  print $0; else if(  $3 == "07" &&  $4 >=first_date ) print $0;}' > $TARGET_DIR/$$_tmp&
  bzcat /data0/match/orig/${V_DATE}/user_sn.txt.bz2 | grep -e '|10324009|'  -e '|10324010|' | awk -F '|' -v first_date='20100701000000'  '{ if(  $3 == "06" && $5 >= first_date )  print $0; else if(  $3 == "07" &&  $4 >=first_date ) print $0;}' > $TARGET_DIR/$$_tmp&
 ...
 #目标文件
  ...
  gawk -F\| 'BEGIN{
         ...
         # 出报表结果
          qiangxiang_outSucc_file=/data0/match/orig/mm7/$V_DATE/stats_month.wuxian_qianxiang.0 
