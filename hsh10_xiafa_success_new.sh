...
awk 'BEGIN{
    ...
    if(FILENAME==""'${day3}'"_hsh10_xiafa_success.txt"){ 
        ...
        if($5==0){
            ...
            print $2"|"$2 >> "'${day3}'_hsh10_no_success_number.txt"
        }
    }
}
END{
...
if [ ${day3} = ${firstday} ];then
    ...
    cat ${day3}_hsh10_no_success_number.txt > yesterday_hsh10_no_success_number.txt
else
    ...
    if [ -f "/data/chenyj/HSH10_SUCCESS/${day4}/hsh10_no_success_number.txt" ];then
        cat /data/chenyj/HSH10_SUCCESS/${day4}/hsh10_no_success_number.txt ${day3}_hsh10_no_success_number.txt | sort -u > yesterday_hsh10_no_success_number.txt
    else
        cat ${day3}_hsh10_no_success_number.txt > yesterday_hsh10_no_success_number.txt
    fi
fi
...
if [ -f "yesterday_hsh10_no_success_number.txt" ]; then
awk 'BEGIN{
    FS=OFS="|"
}{
    if ( FILENAME == "yesterday_hsh10_no_success_number.txt" ){
        error_datas[$1]=0
    }else{
        if($1 in error_datas){
            error_datas[$1] = 1
...
END{
    for (key in error_datas){
        print key
        if (error_datas[key] == 0){
            print $1 >> "hsh10_no_success_number.txt"
        }
    }
}' yesterday_hsh10_no_success_number.txt hsh10_success_number.txt
fi
