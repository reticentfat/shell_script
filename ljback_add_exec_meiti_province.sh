...
RESULT_FILE1=$TARGET_DIR/tmp_ljback_meiti_add_logs.csv     #采购单每日临时快照（按签约日期）
RESULT_FILE2=$TARGET_DIR/tmp_ljback_meiti_exec_logs.csv    #执行单每日临时快照（按投放日期）
...
END{         
    str="省份编码,省份名称,代理商,业务属性,累计采购单量,累计执行单量,累计签约金额,累计执行金额"
    printf("%s\n",str)
 for(name in fee)
    {
