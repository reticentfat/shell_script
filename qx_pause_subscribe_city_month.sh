...
V_DAY=$(echo $DEAL_DATE | cut -c 7-8)
if [ $V_DAY -ne "02" ];then
    echo "不是月初，日期不符合文件生成日期"
    exit 1
fi
V_DATE=`date  -v-2d -j ${DEAL_DATE}"0000" +%Y%m%d`
...
#数据文件
DATA_FILE=$ORIG_DIR/$V_DATE/data/snapshot/snapshot.txt.bz2
#结果文件
REPORT_FILE=$REPORT_DIR/report.region.qianxiang_pause_subcribe_city.csv
...
if [ ! -f "$REPORT_DIR/report.region.qianxiang_subscribe_city.csv" ];then 
    echo "前向业务月报结果文件不存在"
    exit 1
fi
...
}
{
	...
	if(product_num in product){
		...
		e=mktime(s)
		#时间差=退订时间-上一次订购时间
		if( substr(option_time,1,6)=="'$V_MONTH'" && subscribe_status=="07" && (d-e)<=259200){
...
#判断所需文件是否存在
if [ ! -f "$REPORT_DIR/tmp_one" ];then 
    echo "暂停指标结果文件不存在"
    exit 1
fi

echo "省编码,省名称,市编码,市名称,业务主类,业务名称,业务类型,业务资费,前向包月订购用户数,订购用户数,点播用户数,退订用户数,付费用户数,上月新增包月用户数,上月新增留存用户数,本月留存包月用户数,上月包月用户数,本月留存点播用户数,上月点播用户数,四川业务类型,四川业务名称,四川业务资费,暂停用户数,本月开通72小时退订用户数" > $REPORT_FILE
 
awk -F\, '{
    if(FILENAME=="'$REPORT_DIR/'tmp_one"){
...
}' $REPORT_DIR/tmp_one $REPORT_DIR/report.region.qianxiang_subscribe_city.csv |awk -F\, '$1!="省编码"{print}'>>$REPORT_FILE