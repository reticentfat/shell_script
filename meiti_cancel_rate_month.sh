...
V_DATE=$1
#上月最后1天
STARTDATE=$(date -v-1d -j ${V_DATE}"0000" +%Y%m%d )
TIME_FLAG="month"

#判断月报
TEMPDAY=`echo $V_DATE|cut -c 7-8`

[ $TIME_FLAG = 'month' -a $TEMPDAY != '01' ] && exit 0

V_YEAR=$(echo $STARTDATE | cut -c 1-4)
V_MONTH=$(echo $STARTDATE | cut -c 1-6)

#上上月最后1天 = 上月第1天 - 1
Last_day=$(date -v-1d -j $V_MONTH"010000" +%Y%m%d )
...
usa_log=${AUTO_SRC_DATA}/${V_MONTH}*/usa2-biz.log.*${V_MONTH}*.bz2
