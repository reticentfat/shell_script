...
CODE_DIR='/data/match/orig/profile'
  ...
  FILE_APP_CODE=$CODE_DIR/qiangxiang_sms_app_code.txt
 ...
 #目标文件
   TARGET_FILE=$TARGET_DIR/qiangxiang_sms_pay_users.txt

  gawk -F\| 'BEGIN{
                   Current_date="'${V_DATE}'235959";Month=substr(Current_date,1,6)"00000000";Code_File_nodist="'$CODE_DIR'/nodist.tsv"
                   Code_File_appcode="'$CODE_DIR'/qiangxiang_sms_app_code.txt"
                 }
                 {
         ...
         # 出报表结果
          ...
          awk -F'[|,]' '{
                  ...
                  else if(FILENAME=="'$qiangxiang_outSucc_file'")
                  {
           		...
           		printf("%s\n","provinceno,province,appcode,feeusers") >"'$TARGET_DIR'/qiangxiang_sms_pay_province.csv"
           		printf("%s\n","provinceno,province,citycode,city,appcode,feeusers") >"'$TARGET_DIR'/qiangxiang_sms_pay_city.csv"
           		printf("%s\n","provinceno,province,appcode,channel,feeusers") >"'$TARGET_DIR'/qiangxiang_sms_pay_chanel_province.csv"
          		printf("%s\n", "provinceno,province,citycode,city,appcode,channel,feeusers") >"'$TARGET_DIR'/qiangxiang_sms_pay_chanel_city.csv"
                        printf("%s\n","all,appcode,feeusers") >"'$TARGET_DIR'/qiangxiang_sms_pay_country.csv"
                for(name in province_appcode){
                      printf("%s,%d\n",name,province_appcode[name]) >>"'$TARGET_DIR'/qiangxiang_sms_pay_province.csv"
                    }
                for(name in city_appcode){
                     printf("%s,%d\n",name,city_appcode[name]) >>"'$TARGET_DIR'/qiangxiang_sms_pay_city.csv"
                    }
                for(name in province_chanel){
                    printf("%s,%d\n",name,province_chanel[name]) >>"'$TARGET_DIR'/qiangxiang_sms_pay_chanel_province.csv"
                  }
                for(name in city_appcode_chanel){
                     printf("%s,%d\n",name,city_appcode_chanel[name]) >>"'$TARGET_DIR'/qiangxiang_sms_pay_chanel_city.csv"
                 }
                for(name in country){
                     printf("%s,%s,%d\n","合计",name,country[name]) >>"'$TARGET_DIR'/qiangxiang_sms_pay_country.csv"
                 }
