...
}
{
	...
	if(product_num in product){
		...
		e=mktime(s)
		#时间差=退订时间-上一次订购时间
		if( substr(option_time,1,8)=="'$V_DATE'" && subscribe_status=="07" && (d-e)<=259200){
...
echo "省编码,省名称,市编码,市名称,业务主类,业务名称,业务类型,业务资费,前向订购用户数,订购用户数,点播用户数,退订用户数,计费用户数,四川业务类型,四川业务名称,四川业务资费,暂停用户数,本月开通72小时退订用户数" > $REPORT_FILE
...
}' $REPORT_DIR/tmp_one $AUTO_DATA_REPORT/$V_YEAR/$V_DATE/report.region.qianxiang_subscribe_city.csv |awk -F\, '$1!="省编码"{print}'>>$REPORT_FILE
