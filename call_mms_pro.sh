...
if [ $FILEFLAG -eq 1 ];then
   ...
   if [ -f $v_sms_dir/1.zip ];then
      $v_work_dir/sendmms.sh $TELFLAG $v_work_dir/phone_mms.txt $v_sms_dir/mms_msg${DEALDATE}.zip      #$v_work_dir/sendmail.sh $TELFLAG $v_work_dir/q1.txt $v_sms_dir/div_province_day7_${i}_${DEALDATE}.txt $v_daytitle $v_urlfile
...
#9：30
   ...
   #check the daymsg everyday
     if [ -f $v_sms_dir/1.zip ];then
       $v_work_dir/sendmms.sh $TELFLAG $v_work_dir/phone_mms.txt $v_sms_dir/mms_msg${DEALDATE}.zip
     else
       echo "mms_msg${DEALDATE}.zip not exist"
     fi
fi
