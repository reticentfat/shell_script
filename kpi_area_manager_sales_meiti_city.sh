...
CODE_DIR=$AUTO_DATA_NODIST
 ...
 if [ ! -f "$DATA_FILE" ]; then
  ...
  # 结果文件
  RESULT_FILE=$REPORT_DIR/report.region.kpi_area_manager_sales_meiti_city.csv
     gawk -F\| '{
...
str="省份编码,省份名称,地市编码,地市名称,正常指标用户数,总指标用户数"
  printf("%s\n",str)
...
}' $FILE_NODIST $FILE_APP_CODE $DATA_FILE > $RESULT_FILE
